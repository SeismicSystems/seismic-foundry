...
use revm::{
    ...
    interpreter::{
        interpreter::EthInterpreter, return_ok, CallInputs, CallOutcome, CallScheme,
...
pub fn new_evm_with_inspector<'i, 'db, I: InspectorExt + ?Sized>(
    ...
    let ctx = EthEvmContext {
        journaled_state: {
            let mut journal = Journal::new(env.evm_env.cfg_env.spec, db);
...
fn get_precompiles(spec: SpecId) -> Precompiles {
    Precompiles::from_static(
        EthPrecompiles {
