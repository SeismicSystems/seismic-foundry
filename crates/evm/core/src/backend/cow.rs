...
use crate::{
    ...
    }, fork::{CreateFork, ForkId}, Env, EnvMut, InspectorExt
};
...
use revm::{
    context::result::ResultAndState, database::DatabaseRef, primitives::map::HashMap as Map, state::{
        Account, AccountInfo, Bytecode, FlaggedStorage, 
...
impl<'a> CowBackend<'a> {
    ...
    pub fn inspect<I: InspectorExt>(
        ...
        let mut evm = crate::evm::new_evm_with_inspector(self, env.clone(), inspector);
    ...
    fn backend_mut(&mut self, env: &Env) -> &mut Backend {
        if !self.is_initialized {
            ...
            let env = Env::new_with_spec_id(env.evm_env.cfg_env, env.evm_env.block_env, env.evm_env.tx_env, self.spec_id);
