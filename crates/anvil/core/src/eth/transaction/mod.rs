...
use seismic_transaction::{
    encoding_decoding::{
        decode_enveloped_seismic_tx, encode_2718_len, encode_2718_seismic_transaction
    },
...
impl TypedTransaction {
    ...
    pub fn gas_price(&self) -> u128 {
        match self {
            ...
            Self::Deposit(_) => 0,
            Self::Seismic(req) => u128::try_from(req.tx().tx.gas_price).unwrap(),
        }
    }
    ...
    pub fn value(&self) -> U256 {
        U256::from(match self {
            ...
            Self::Seismic(tx) => tx.tx().tx.value,
        })
    }
...
impl Decodable for TypedTransaction {
    fn decode(buf: &mut &[u8]) -> alloy_rlp::Result<Self> {
        ...
        println!("Type = {:?}", ty);
        if ty == 0x4A {
            println!("TypedTransaction.decode on seismic tx");
            Ok(Self::Seismic(decode_enveloped_seismic_tx(buf)?))
        }
        else if ty != 0x7E {
...
impl Decodable2718 for TypedTransaction {
    fn typed_decode(ty: u8, buf: &mut &[u8]) -> Result<Self, Eip2718Error> {
        match ty {
            ...
            0x4A => {
                println!("TypedTransaction.typed_decode on seismic tx");
                return Ok(Self::Seismic(decode_enveloped_seismic_tx(buf)?))
            },
            _ => {}
        }
...
impl From<TypedReceipt<alloy_rpc_types::Log>> for OtsReceipt {
    fn from(value: TypedReceipt<alloy_rpc_types::Log>) -> Self {
        ...
        let logs = receipt.receipt.logs.into_iter().collect();
