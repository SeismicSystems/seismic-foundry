...
async fn test_seismic_transaction() {
    ...
    let commitment_key = compute_commitment_key(value, to);
    let commitment =
        anvil::eth::SEISMIC_DB.clone().get_commitment(commitment_key).unwrap().unwrap();
    let commit_value = U256::from_be_slice(&commitment.secret.value.as_ref());
    assert_eq!(commit_value, value);
    ...
    // Check whether the value 30 has been committed to the DB for this contract.
