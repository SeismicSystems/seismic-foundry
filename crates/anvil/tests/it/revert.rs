...
use reqwest::Url;
use seismic_prelude::foundry::{SeismicProviderBuilder, SignedProviderExt, tx_builder};
...
async fn test_solc_revert_example() {
    ...
    let err =
        seismic_provider.seismic_call_raw(SendableTx::Builder(builder.into())).await.unwrap_err();
